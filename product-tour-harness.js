(() => {
      ...
      steps: [
        ...
        },
        {
          ...
          html: `${head('Processing pipeline','What happens after upload','The original record and extracted data stay visibly connected.')}
            <div class="pth-process"><article class="is-done"><b>1 · Validate</b><span>Confirm file type, size and page structure.</span></article><article class="is-done"><b>2 · Read</b><span>Recognize text, tables, dates and clinical context.</span></article><article class="is-done"><b>3 · Classify</b><span>Sort findings into the correct health data type.</span></article><article><b>4 · Link</b><span>Attach each item to its record and source page.</span></article></div>
      ...
      steps: [
        {
          ...
          html: `${head('Clinical review','Three issues need attention','Prioritized by how the issue could affect the patient history or report.','<span class="pth-chip amber">3 open</span>')}
  ...
  if (scrollHero) {
    ...
    const storyPatentNote = heroStory?.querySelector('.patent-pending-note');
    ...
    const heroMenuDefinitions = [
      ...
      { view: 'patient-notes', label: 'Patient Notes', kicker: 'Patient note', title: 'Keep important context with the patient.', lead: 'Add one patient-level note for context the report generator should take into account.', bullets: ['Keep one current note per patient','Edit the note when the context changes','Use it to guide report generation'], badge: '1 note' },
      ...
      { view: 'genetics', label: 'Genetics', kicker: 'Genetic findings', title: 'Put genetic findings in clinical context.', lead: '', bullets: ['Separate raw DNA from lab data','Review findings by evidence tier','Expand each gene for variant context'], badge: '12 findings', action: 'Upload Records' },
      { view: 'medication-timeline', label: 'Medications', editable: true, kicker: 'Medication history', title: 'Review the medication history, notes and timeline.', lead: 'See current and past medications across time. Review dates, status, dosage and notes, then open a medication to edit it.', bullets: ['See when each medication started and ended','Review current and stopped treatments','Open the medication notes and details'], badge: '7 medications', action: '+ Add' },
    ...
    const clinicianPageSummaries = {
      ...
      'Patient Notes': 'Keep important patient context in one place.',
      'Medical Records': 'Upload records. n1 builds the history.',
      Reports: 'Create, review and approve each report.',
      Biomarkers: 'See how the result developed over time.',
      Diagnoses: 'Review the documented diagnosis with its history.',
      Procedures: 'See the investigations that shaped the history.',
      Genetics: 'Put genetic findings in clinical context.',
      Medications: 'Review the medication history, notes and timeline.',
      Supplements: 'Bring supplements into the clinical review.'
    };
    const clinicianPageLeads = {};
    ...
    const renderHeroRecordUpload = () => {
      const records = [
        ...
        ['Example Imaging Center 01','cardiac-ct-2024.pdf','Imaging','22 Nov 2024']
      ];
      const status = `<em class="hero-upload-live"><i></i><span><u>Preparing</u><u>Reading</u><u>Organizing</u><u>Complete</u></span></em>`;
    ...
    const renderHeroPatientNotes = () => `<div class="hero-patient-notes-demo">
      <header><div><b>Patient Notes</b><span>Note about John Doe</span></div><em>Saved</em></header>
      <article class="hero-patient-note-card">
        <div><b>Patient note</b><span>Taken into account by the report generator when creating this patient's reports</span></div>
        <p>Patient prefers conservative treatment approaches. Allergic to penicillin.</p>
        <footer><small>One note is kept for this patient.</small><b>Edit note</b></footer>
      </article>
    </div>`;
    ...
    const renderHeroMedicationTimeline = () => `<div class="hero-medication-timeline">
      <div class="hero-medication-tools"><span class="is-search">Search medications…</span><div><b class="is-selected">All</b><span>Current</span><span>Stopped</span></div><button type="button">1Y</button><button class="is-selected" type="button">All</button></div>
      <div class="hero-medication-axis"><span></span><div><i style="left:4%">2023</i><i style="left:31%">2024</i><i style="left:58%">2025</i><i style="left:84%">2026</i><em style="left:91%">TODAY</em></div><div><i>STARTED</i><i>ENDED</i><i>STATUS</i></div></div>
      <div class="hero-medication-rows">
        <article><header><b>Metformin</b><small>500 mg · twice daily</small><u>Review notes</u></header><div class="hero-medication-track"><i class="year-2023"></i><i class="year-2024"></i><i class="year-2025"></i><i class="today"></i><span class="bar is-current" style="--start:24%;--length:67%"><em>Metformin</em></span></div><footer><time>22 Nov 2023</time><time>—</time><b>Current</b></footer></article>
        <article><header><b>Rosuvastatin</b><small>10 mg · daily</small><u>Lipid management</u></header><div class="hero-medication-track"><i class="year-2023"></i><i class="year-2024"></i><i class="year-2025"></i><i class="today"></i><span class="bar is-current" style="--start:58%;--length:33%"><em>Rosuvastatin</em></span></div><footer><time>14 Mar 2025</time><time>—</time><b>Current</b></footer></article>
        <article><header><b>Aspirin</b><small>81 mg · daily</small><u>Previous treatment</u></header><div class="hero-medication-track"><i class="year-2023"></i><i class="year-2024"></i><i class="year-2025"></i><i class="today"></i><span class="bar is-stopped" style="--start:39%;--length:27%"><em>Aspirin</em></span></div><footer><time>22 Nov 2024</time><time>18 Feb 2025</time><b class="is-stopped">Stopped</b></footer></article>
      </div>
      <div class="hero-medication-legend"><span><i></i>Current medication</span><span><i></i>Stopped medication</span><small>Drag a bar or select a date to update the medication history.</small></div>
    </div>`;
    const renderHeroPageBody = (definition) => {
      if (definition.view === 'dashboard') return `<div class="product-hero-dashboard-stats">${definition.stats.map((stat) => `<article><i></i><div><b>${stat[1]}</b><span>${stat[0]}</span></div></article>`).join('')}</div><div class="product-hero-dashboard-body"><section><header><b>Quick Actions</b></header><div class="product-hero-quick-actions"><article><i>↑</i><div><b>Upload Records</b><span>Add PDFs and supported data files</span></div><strong>→</strong></article><article><i>▤</i><div><b>Generate Report</b><span>Choose a report and set its scope</span></div><strong>→</strong></article></div></section><section><header><b>Recent Reports</b><span>View all →</span></header><div class="product-hero-dashboard-list"><article><b>Health Summary</b><span>26 Aug 2026</span><strong>Complete</strong></article><article><b>Health Over Time</b><span>25 Aug 2026</span><strong>Generating</strong></article></div></section><section><header><b>Biomarker Alerts</b><span>2 need review</span></header><div class="product-hero-dashboard-list"><article><b>LDL cholesterol</b><span>116 mg/dL · target ≤100</span><strong>Review</strong></article><article><b>Vitamin D</b><span>71 nmol/L · range 50–125</span><strong>In range</strong></article></div></section></div>`;
      if (definition.view === 'patient-notes') return renderHeroPatientNotes();
      ...
      if (definition.label === 'Medications') return renderHeroMedicationTimeline();
    ...
    const chapters = heroStory?.querySelector('.product-hero-chapters');
    if (chapters) {
      chapters.innerHTML = heroMenuDefinitions.map((definition, index) => `<li><button type="button" data-hero-chapter="${index}">${definition.label}</button></li>`).join('');
      chapters.addEventListener('click', (event) => {
        const button = event.target.closest('[data-hero-chapter]');
        if (!button) return;
        const index = Number(button.dataset.heroChapter);
        const scrollDistance = Math.max(1, scrollHero.offsetHeight - innerHeight);
        const progress = .24 + ((index + .4) / heroMenuDefinitions.length) * .72;
        window.scrollTo({ top: scrollHero.offsetTop + progress * scrollDistance, behavior: matchMedia('(prefers-reduced-motion: reduce)').matches ? 'auto' : 'smooth' });
      });
    }
    ...
    const setHeroMenu = (index) => {
      ...
      heroSidebarItems.forEach((item) => item.classList.toggle('is-active', item.textContent.trim() === definition.label));
      chapters?.querySelectorAll('[data-hero-chapter]').forEach((button, buttonIndex) => {
      ...
      if (scrollHero.classList.contains('clinician-product-hero')) {
        ...
        if (storyLead) {
          const lead = clinicianPageLeads[definition.label] || '';
          storyLead.textContent = lead;
          storyLead.hidden = !lead;
        }
        if (storyList) storyList.innerHTML = '';
        if (storyPatentNote) storyPatentNote.hidden = true;
      }
    };
    const resetHeroMenu = () => {
      heroMenuIndex = -1;
      ...
      if (storyLead) {
        storyLead.textContent = originalStory.lead;
        storyLead.hidden = false;
      }
      if (storyList) storyList.innerHTML = originalStory.list;
      if (storyPatentNote) storyPatentNote.hidden = false;
    };
